...
func RunQEMU(proj *osbstar.Project, unitName, machineName, projectDir string, opts QEMUOptions, w io.Writer) error {
	...
	if _, err := exec.LookPath(qemuBin); err == nil {
		...
		// the image launches into SeaBIOS (which would boot nothing, since
		// UEFI images carry no MBR boot code). The common packages are:
...
// ovmfFirmware returns the path to an OVMF (x86_64 UEFI) firmware image
// usable with `-bios` to UEFI-boot an x86_64 guest, or "" if none is
...
func baseQEMUArgs(machine *osbstar.Machine, opts QEMUOptions) []string {
	...
	} else if qemu != nil && qemu.Firmware != "" {
		switch qemu.Firmware {
		case "ovmf":
			// Path detected by ovmfFirmware(); the RunQEMU preflight already
			// verified it exists and reported a clear error if not, so a
			// missing firmware here (TUI preview path) is silently omitted
			// rather than aborting the preview.
			if fw := ovmfFirmware(); fw != "" {
