...
// TestBaseQEMUArgsSecureBoot checks the Secure Boot firmware wiring: the
// machine gains smm=on (required for the protected variable store), and when a
// Secure-Boot-capable OVMF is present the pflash CODE/VARS pair and the
// `secure` global replace the plain -bios. The pflash assertions are guarded
...
func TestBaseQEMUArgsSecureBoot(t *testing.T) {
	...
	if !containsPair(args, "-machine", "q35,smm=on") {
	...
	if !containsPair(args, "-global", "driver=cfi.pflash01,property=secure,value=on") {
		t.Errorf("SecureBoot: expected secure pflash global, got %v", args)
	}
	if slices.Contains(args, "-bios") {
		t.Errorf("SecureBoot: expected no -bios (pflash used instead), got %v", args)
	}
	foundVars := false
	for _, a := range args {
		if strings.Contains(a, "if=pflash") && strings.Contains(a, "/run/OVMF_VARS.sb.fd") {
			foundVars = true
		}
	}
	if !foundVars {
		t.Errorf("SecureBoot: expected enrolled vars as pflash unit=1, got %v", args)
	}
}
