...
def image(name, artifacts=[], distro_artifacts={}, hostname=None, timezone="", locale="",
    ...
    if _is_apt_distro(effective_distro):
        ...
        disk_fn = (lambda: _create_disk_image_uefi_debian(name, all_partitions)) if uefi else (lambda: _create_disk_image_debian(name, all_partitions))
    else:
...
def _create_disk_image_uefi(name, partitions):
    ...
    packages installed (via distro_packages in the machine definition).
...
def _create_disk_image_uefi_debian(name, partitions):
    ...
    Mirrors _create_disk_image_uefi but runs in the glibc toolchain container.
    grub-efi-amd64 (and its grub-common/grub2-common dependencies) provide
    grub-mkimage and the x86_64-efi module directory.
    """
    if not partitions:
        return
    ...
    # Generate extlinux.conf placeholder is NOT needed for UEFI; GRUB reads
    # its own grub.cfg from the ESP. The rootfs task already ran depmod and
    # update-initramfs before this disk task.
    ...
    # No syslinux MBR to install; UEFI firmware finds BOOTX64.EFI on the ESP.
